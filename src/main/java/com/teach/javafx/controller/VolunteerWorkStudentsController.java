...
public class VolunteerWorkStudentsController extends ToolController {
    @FXML
    ...
    private Integer volunteerWorkId = null; // 志愿活动ID

    @FXML
    public void initialize(Integer volunteerWorkId) {
        ...
        // 根据志愿活动ID获取学生列表
        getStudentListByVolunteerWorkId();
        // 设置表格列值工厂
    ...
    private void getStudentListByVolunteerWorkId() {
        DataResponse res;
        DataRequest req = new DataRequest();
        req.add("volunteerWorkId", volunteerWorkId);
        res = HttpRequestUtil.request("/api/volunteerWork/getVolunteerWorkStudents", req);
    ...
    protected void onStudentsQueryButtonClick() {
        ...
        if (searchText.isEmpty()) {
            ...
            req.add("volunteerWorkId", volunteerWorkId);
            DataResponse res = HttpRequestUtil.request("/api/volunteerWork/getVolunteerWorkStudent", req);
            if (res != null && res.getCode() == 0) {
                studentList = (ArrayList<Map>) res.getData();
                setTableViewData();
            }
    ...
    protected void onStudentsDeleteButtonClick() {
        ...
        req.add("volunteerWorkId", volunteerWorkId);
        DataResponse res = HttpRequestUtil.request("/api/volunteerWork/deleteVolunteerWorkStudent", req);
        if (res != null) {
            if (res.getCode() == 0) {
    ...
    protected void onStudentsSaveButtonClick() {
        ...
        DataResponse res;
        if (isAdd) {
            res = HttpRequestUtil.request("/api/volunteerWork/addVolunteerWorkStudent", req);
        }
        else {
//            DataResponse addres = HttpRequestUtil.request("/api/volunteerWork/addVolunteerWorkStudent", req);
//            if (addres != null && addres.getCode() == 0) {
//                DataRequest delReq = new DataRequest();
//                delReq.add("personId", personId);
//                delReq.add("volunteerWorkId", volunteerWorkId);
//                res = HttpRequestUtil.request("/api/volunteerWork/deleteVolunteerWorkStudent", delReq);
//            }
//            else {
//                MessageDialog.showDialog("修改失败：" + addres.getMsg());
//                return;
//            }
            req.add("personId", personId);
            res = HttpRequestUtil.request("/api/volunteerWork/editVolunteerWorkStudent", req);
        }
